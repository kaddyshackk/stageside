...
namespace StageSide.SpaCollector.Service.Operations.CreateSpaConfig
{
    public class CreateSpaConfigEndpoint : IEndpoint
    {
        ...
        private static async Task<Results<Created<CreateSpaConfigResponse>, BadRequest<string>>> HandleAsync(CreateSpaConfigRequest request, ConfigurationService configuration, CancellationToken ct)
        {
            var command = new CreateSpaConfigRequestMapper().MapToCommand(request);
            var config = await configuration.CreateCollectionConfigAsync(command, ct);
