...
namespace ComedyPull.Application.Modules.Punchup.Processors
{
        ...
        : ISubProcessor<DataSourceType>
    {
        ...
        public async Task ProcessAsync(IEnumerable<BronzeRecord> records, CancellationToken cancellationToken)
        {
            ...
            using (LogContext.PushProperty("BronzeRecordCount", bronzeRecords.Length))
            {
                foreach (var bronzeRecord in bronzeRecords)
                {
                    using (LogContext.PushProperty("RecordId", bronzeRecord.Id))
                    {
                        PunchupRecord? punchupRecord;
                        try
                        {
                            punchupRecord = JsonSerializer.Deserialize<PunchupRecord>(bronzeRecord.Data);
                        }
                        catch (JsonException ex)
                        {
                            logger.LogError(ex, "Failed to deserialize PunchupRecord from BronzeRecord {RecordId}", bronzeRecord.Id);
                        ...
                        {
                            logger.LogWarning("Failed to deserialize PunchupRecord from BronzeRecord {RecordId}", bronzeRecord.Id);
                            bronzeRecord.Status = ProcessingStatus.Failed;
                            continue;
                        }
                        ...
                        silverRecordsToCreate.AddRange(from punchupEvent in punchupRecord.Events
                ...
                {
                    using (LogContext.PushProperty("SilverRecordCount", silverRecordsToCreate.Count))
                    {
                        await repository.CreateSilverRecordsAsync(silverRecordsToCreate, cancellationToken);
                        logger.LogDebug("Created {Count} SilverRecords from {BronzeCount} BronzeRecords",
